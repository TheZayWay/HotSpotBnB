...
const CreateReviewForSpot = ({ spotId, sessionUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const commentLength = review.length;
    setIsCommentValid(commentLength >= 10);
    setIsRatingValid(stars !== 0);

    if (commentLength >= 10 && stars !== 0) {
      const reviewDetails = {
        review,
        stars,
      };
  ...
  return (
    <div className="create-review-container">
      <h3 className="textarea-title">How was your stay?</h3>
      <div className="outer-rev-form-container">
        <form className="review-form-container" onSubmit={handleSubmit}>
          <ul className="errors-map">
            {errors.map((error) => (
              <li key={error}>{error}</li>
          ...
          <div className="review-input-container">
            <label>
              <textarea
                ...
                disabled={isButtonDisabled}
                className={`review-text-area ${
                  !isCommentValid && "invalid-comment"
                }`}
                placeholder="Leave your review here"
              />
            </label>
            ...
            {!isRatingValid && (
              <p className="validation-error">Please select a rating</p>
            )}
            <button
              disabled={!isCommentValid || !isRatingValid || isButtonDisabled}
              type="submit"
